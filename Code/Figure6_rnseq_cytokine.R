...
group_media._RPMI_5..FBS_vs_auto.plasma <- results(cds, name="group_media._RPMI_5..FBS_vs_Vehicle",alpha=0.05)
group_media._RPMI_5..FBS_vs_auto.plasma <- as.data.frame(lfcShrink(cds, coef = 2, res = group_media._RPMI_5..FBS_vs_auto.plasma,type='apeglm'))
...
sigGenes <- unique(c(
  ...
  rownames(subset(group_media._RPMI_5..FBS_vs_auto.plasma, padj<=qvaluecutoff & abs(log2FoldChange)>=log2cutoff & baseMean > mean_counts_thresh))))
...
data<-fread("~/Luminex/luminex_vehicle_sara.csv",header=T)#,sep=",")
